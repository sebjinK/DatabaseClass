...
-- Apply a discount to all tickets
DROP TRIGGER trgApplyDiscount;
CREATE TRIGGER trgApplyDiscount
BEFORE INSERT ON tblTickets
FOR EACH ROW
SET NEW.total_amount = NEW.total_amount*0.8;

INSERT INTO tblTickets (customer_id, event_id, purchase_date, quantity, total_amount)
VALUES (3, 2, '2025-10-20',1,100.00);

SELECT * FROM tblTickets;
SELECT * FROM tblEvents;

-- Apply a markup on all events
CREATE TRIGGER trgMarkup
BEFORE INSERT ON tblEvents
FOR EACH ROW
SET NEW.ticket_price = NEW.ticket_price*1.15;
DROP TRIGGER trgPriceIncrease;

-- delete all tickets
DELIMITER $$
CREATE PROCEDURE prcDeleteAllTickets()
BEGIN
	DELETE FROM tblTickets;
END $$
DELIMITER ;

CALL prcDeleteAllTickets;
SELECT * FROM tblTickets;
DROP TABLE tblTickets;

-- Increase ticket prices by 3 times the original amount
DELIMITER $$
CREATE PROCEDURE prcTicketMaster()
BEGIN
	UPDATE tblEvents SET ticket_price = ticket_price * 3;
END $$
DELIMITER ;
CALL prcTicketMaster;
SELECT * FROM tblEvents;

SHOW PROCEDURE STATUS;

-- set new inserted name to be yours (big for cybersecurity)
CREATE TRIGGER trgChangeName
BEFORE INSERT ON tblCustomers 
FOR EACH ROW
	SET NEW.first_name = 'Sebjin', NEW.last_name = 'Kennedy';
    
INSERT INTO tblCustomers (first_name, last_name, email, membership_level)
VALUES ('Bob', 'Doe', 'bdoe@gmail.com', 'Gold');

SELECT * FROM tblCustomers;

DROP TRIGGER trgChangeName;
